...
import {chatHrefConstructot} from '@/lib/utils'
	...
	const friendsWithLastMessage = await Promise.all(
			friends.map(async (friend) => {
				const [lastMessageRaw] = (await fetchRedis(
					'zrange',
					`chat:${chatHrefConstructot(session.user.id, friend.id)}:messages`,
					-1,
					-1
				)) as string[]
			...
			{friendsWithLastMessage.length === 0 ? (
				...
				friendsWithLastMessage.map(friend => (
					...
					>
						...
						<Link
							href={`/dashboard/chat/${chatHrefConstructot(session.user.id, friend.id)}`}
